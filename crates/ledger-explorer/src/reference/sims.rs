...
/// mini-cloud-az-double-assign: AZ-aware placement fence missing (cloud-infra class).
///
/// Control-plane scheduler (task 0) assigns shard 42 to a worker in AZ-a
/// (task 1) and to a worker in AZ-b (task 2) without cross-AZ fencing.
/// Both workers apply the same shard id. Distinctness fails because the same
/// shard is owned in two AZs at once: the classic placement-fence bug seen
/// in multi-AZ control planes when the AZ membership view is stale.
/// Mirrors the double-grant shape but the cause is stale AZ isolation state.
pub fn mini_cloud_az_double_assign() -> (Vec<TaskBuilder>, impl Fn(&Journal) -> bool) {
    let builders: Vec<TaskBuilder> = vec![
        Box::new(|b: Boundary| {
            task(async move {
                let _ = b.send(1, 42);
...
/// mini-cloud-instance-flap: instance flap duplicate registration (cloud-infra class).
///
/// Client (task 0) registers instance 777 with the autoscaler (task 1),
/// which forwards to the durable instance registry (task 2) and acks the
/// client before its dedup state is durable. A simulated crash-restart
/// replays the WAL without dedup and forwards the same registration again,
/// so the registry carries the instance twice. Distinctness fails.
/// Mirrors the crash-restart duplicate-append shape at the cloud instance
/// lifecycle level.
pub fn mini_cloud_instance_flap() -> (Vec<TaskBuilder>, impl Fn(&Journal) -> bool) {
    let builders: Vec<TaskBuilder> = vec![
        Box::new(|b: Boundary| {
            task(async move {
                let _ = b.send(1, 777);
        ...
        Box::new(|b: Boundary| {
            task(async move {
                let instance = b.recv().await;
                let _ = b.send(2, instance);
                let _ = b.recv().await;
                let _ = b.send(0, 1);
                let _ = b.send(2, instance);
                let _ = b.recv().await;
...
/// mini-cloud-config-drift: staged rollout divergence (cloud-infra class).
///
/// Rollout coordinator (task 0) pushes config version 5 to both fleet nodes
/// (tasks 1 and 2), then upgrades only node-b to version 9 after a short
/// delay and never reconciles node-a. Node-a keeps serving config 5 while
/// node-b and the coordinator hold 9. Convergence fails: the fleet serves
/// divergent configs after a partial rollout.
pub fn mini_cloud_config_drift() -> (Vec<TaskBuilder>, impl Fn(&Journal) -> bool) {
    let builders: Vec<TaskBuilder> = vec![
        Box::new(|b: Boundary| {
            task(async move {
                let _ = b.send(1, 5);
...
/// mini-cloud-quota-retry-storm: quota backpressure duplicate apply (cloud-infra class).
///
/// Client (task 0) requests quota ticket 88. The quota service (task 1)
/// applies it but its ack is held by a backpressure partition window on the
/// service-to-client link. The client's retry timer fires, the partition
/// heals, and the client retries the same ticket without an idempotency key.
/// The service applies every received ticket without dedup, so ticket 88
/// applies twice. Distinctness fails: backpressure plus at-least-once retry
/// without dedup becomes a cloud control-plane duplicate-execution bug.
pub fn mini_cloud_quota_retry_storm() -> (Vec<TaskBuilder>, impl Fn(&Journal) -> bool) {
    let builders: Vec<TaskBuilder> = vec![
        Box::new(|b: Boundary| {
            task(async move {
                let _ = b.send(1, 88);
                record_partition(&b, 1, 0);
                b.sleep(Duration::from_micros(2)).await;
                record_partition(&b, 1, 0);
                let _ = b.send(1, 88);
                let _ = b.recv().await;
            })
